...
const DJ_PASSWORD = 'puerto2026'; // Contraseña del panel DJ

// Websockets
io.on('connection', (socket) => {
  // Cuando se conecta un cliente (DJ o Screen), enviar estado inicial si es necesario
  
  socket.on('dj-login', (pass) => {
    if (pass === DJ_PASSWORD) {
      socket.emit('login-response', { success: true });
    } else {
      socket.emit('login-response', { success: false, message: 'Contraseña incorrecta' });
    }
  });
